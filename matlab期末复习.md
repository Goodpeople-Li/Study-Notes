...
v = start:step:end;
```
生成一个从start开始到end结束，步长为step的行向量v。
...
```
作用：对两个符号多项式做“整除”式的多项式除法，返回商 Q 与余数 R；语法 [Q,R] = quorem(A,B,x) 视 A,B 为关于变量 x 的多项式。余数 R 的次数 < 除式 B 的次数。与 deconv 的区别：quorem 面向符号表达式（直接用 x），deconv 面向系数向量。若只需商或余，可分别用 Q = quorem(A,B,x) 或 [~,R] = quorem(A,B,x)
